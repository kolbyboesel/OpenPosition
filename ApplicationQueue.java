...
public class ApplicationQueue implements PriorityQueueADT<Application>, Iterable<Application> {
	...
	private void percolateDown(int i) {
		...
		for (; i * 2 <= size; i = childIndex) {
			childIndex = i * 2;
			if (childIndex != size && queue[childIndex + 1].compareTo(queue[childIndex]) < 0)
				childIndex++;
			if (queue[childIndex].compareTo(temp) < 0)
				queue[i] = queue[childIndex];
			else
				break;
		}
		queue[i] = temp;
