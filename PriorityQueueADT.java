 ...
 *
 * @param <T> type parameter bounded by the Comparable interface. Represents the
 *            type of elements stored in this priority queue.
 */
public interface PriorityQueueADT<T extends Comparable<T>> {
	...
	/**
	 * Returns and removes the element at the front (aka root position) of this
	 * queue (the element having the highest priority).
	 *
	 * @return the removed element
	 * @throws NoSuchElementException with a descriptive error message if this queue
	 *                                is empty
	 */
	public T dequeue();

	/**
	 * Returns without removing the element at the front (aka root position) of this
	 * queue (the element having the highest priority).
	 *
